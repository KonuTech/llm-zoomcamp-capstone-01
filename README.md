...
The inputs from my example:
```
? Enter your question: Is the game worth the price if it's performance in considered?
? Enter game title: God of War: Ragnarok
```

At the moment, the Elasticsearch database stores reviews for about 22 computer games. Please feel free to pull reviews for different games and try to index them by yourself as well. Currently, the logic responsible for pulling the reviews, stored as JSON documents, is implemented in a [notebook](https://github.com/KonuTech/llm-zoomcamp-capstone-01/blob/main/notebooks/001_rag_test_002.ipynb). Add the game title to the list, move or copy a JSON file to ```./backend/app/data/reviews```, and reindex the knowledge base with ```python3 backend/app/prep.py```.

Here are few samples of my Q&A prompts:
